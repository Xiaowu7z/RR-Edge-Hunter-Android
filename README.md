...
三个策略均先从每族最多 100 个候选中进行三次 TCP connect 快筛，任一轮失败即淘汰。均衡与亚洲狩猎让低 RTT 前 10 名进入严格下载验证，并在某个 IP 连续两次达到目标带宽后提前结束；最大带宽保留 20 个多样候选，避免只看最低延迟漏掉更高吞吐线路。最终推荐均须完成至少两次成功的严格下载样本。
...
5. 均衡/亚洲狩猎选择低 RTT 前 10；最大带宽选择低 RTT 主体与跨延迟分位共 20 个，避免纯延迟截断。
