...
优选出的裸 IP 只用于替换节点的 `address/server`。节点端口、UUID、协议、SNI、Host 与 WS Path 应保持原配置。应用要求用户粘贴自己已有的 VMess/VLESS WS+TLS 节点，并把原端口、SNI、Host 与 Path 作为最终路由门禁；完整节点链接与 UUID 不保存。
